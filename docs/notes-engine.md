...
- 进度:
  ...
  - **E-CROWD11 v2**（`server/src/crowd-aggregate.ts` + `crowd-normalize.ts` + `crowd-import.ts` + `crowd-aggregate.test.ts` + `routes/data.ts` + `db/data.ts` + 双侧 `types.ts` + `web/src/lib/api/data.ts`）:
    ...
    - **R6 映射值修正**：gender "其他"→"O"（非 OTHER）、preferred_channel "微信"→"wechat_mp"（非 wechat）。
    - **R4 测试重写**：23 用例全绿（v2 宽表+拒绝 v1+demo fixture+归一化 6 条路径）。
    - 路由响应体 + 前端 stub type 同步增加 segments 字段。
    - 端到端验证：demo-aggregate.csv → 6 CrowdSegment ✅；demo-detail.csv → computeFieldProfiles(normalizeEnums:true) → gender topValues [F,M,O] / city_tier [T1,T4_PLUS,T1_NEW,T3,T2] / channel [xhs,jd,offline,douyin,tmall,wechat_mp] / lifecycle [churned,repeat,active,new,dormant] / discount [flash_sale,member_price,coupon,none]，零中文残留 ✅。
    - 双侧 types: `CrowdDatasetSource` 新增 `"aggregate_upload"`、`CrowdDataset` 新增 `isAggregate?`、`CrowdTagDictionaryEntry`/`CrowdSegment` 新增 `autoGenerated?`。
    - 测试 11 用例全绿（聚合导入+字典生成+分群创建+DB持久化+workspace隔离）。
  - **E-CROWD5**（`server/src/crowd-profile-runner.ts` + `crowd-profile-runner.test.ts` + `routes/data.ts`）:
    - 实现 `runCrowdProfileGeneration()`：从 segment 聚合摘要 + businessContext 生成结构化 CrowdProfileContent，直接落 DB（profile + version）。测试 20 用例。
  - **E-CROWD8**（`server/src/simulation-lab.ts` + `web/src/components/SimulationLabPane.tsx` + `simulation-lab.test.ts`）:
    - server：`parseSimulationRunRequest` 新增 `source: "crowd_profile"` 支持，提取 `crowdProfileId` / `crowdProfileVersionId`；payload 只传 persona，不传标签明细/toolIds。
    - frontend：SimulationLabPane 新增 third source 分组「来自 the-crowd 画像」，含搜索框（名称/traits/persona），选中后 lifeForms 带 `crowdProfileId`/`crowdProfileVersionId`。
    - frontend：ResultBlock 每个 crowd_profile 来源的角色评估增加「回写到画像反馈」按钮，调用 `api.createCrowdProfileFeedbackFromSimulation`，反馈进入 ProfileFeedback 待采纳列表。
    - 测试新增 2 用例（parse crowd_profile source + 无 crowdProfileId 降级），总计 28/28。
  - **前序状态仍有效**：DLF 模拟实验专题 done；MONITOR-TARGET1 done；SKILLINJECT1/SUBSKILL1/SKILLOPT1 浏览器 smoke 仍 pending；subagents 进阶/flow_node_runs/E-QEVAL2/SkillLab/E-COLLECT2 浏览器验收仍待执行。
- 校验:
...
- **crowd profile 画像生成 runner（E-CROWD5，2026-06-28）**：`crowd-profile-runner.ts` 遵循 simulation-lab 同源模式（parse → build prompts → LLM → JSON repair → normalize → persist），但产出直接落 DB（`crowd_profiles` + `crowd_profile_versions`），不写 workspace 文件系统 artifact。输入白名单：segment rule + 字段画像聚合摘要 + 标签字典解释 + 标签分布摘要 + 业务场景说明；禁止输入：原始行级标签明细 / 用户级字段值列表 / 错误日志样本 / draw_data。输出 schema = `CrowdProfileContent`（traits/motivations/decisionTriggers/objections/tone/contentPreference/riskNotes/evidenceSummary/persona）。**红线**：① evidenceSummary 正则拦截 row/record/user/individual/sample 等行级引用；② `CrowdFieldProfile` 类型用 `numericRange?: { min, max }` 不是 `numericStats`（已踩坑修正）；③ prompt builder 是纯函数，只接受聚合数据参数，不直接读 DB。前端 stub `engineApi.generateCrowdProfile` 已存在于 `web/src/lib/api/engine.ts:793`，指向 `POST /api/workspaces/:id/crowd/profile-generation/run`，路由已实装在 `routes/data.ts`。**待 D 域前端接入**：CrowdPane / ProfileViewer 需加「生成画像」按钮调用此 API。

- **SimulationLab crowd profile 联动（E-CROWD8，2026-06-28）**：模拟实验数字生命体新增第三来源 the-crowd 画像。server 端 `parseSimulationRunRequest` 扩展 `source` 枚举接受 `"crowd_profile"`，提取 `crowdProfileId`/`crowdProfileVersionId` 并透传（payload 只传 persona，不传标签明细/toolIds，红线不变）。frontend SimulationLabPane 新增 `workspaceId` prop + 第三 card 分组「来自 the-crowd 画像」，含搜索框（名称/traits/persona fuzzy match），选中 profile 后 lifeForms 带 `crowdProfileId`/`crowdProfileVersionId`。ResultBlock 每个 crowd_profile 来源的角色评估增加「回写到画像反馈」按钮，调用 `api.createCrowdProfileFeedbackFromSimulation`（已有端点），反馈进入 `crowd_profile_feedback` 待采纳列表，不自动改 persona。**关键边界**：① 画像加载需 `workspaceId`（scope 为 workspace 类型时直接取，session/flow 类型依赖 EngineTabs 传入 `ctx.activeWorkspaceId`）；② 画像只取 `status=draft|active` 且有 `currentVersionId` 的条目；③ 回写只传 objections/acceptanceConditions/suggestions，不传 evidenceQuotes（证据是报告口径不是人群口径）。

- **聚合数据上传 + 自动字典 + 默认分群（E-CROWD11 v2，2026-06-28）**：三个纯算法端点 + 归一化层，零 LLM 调用。① `POST .../import-aggregate`：接收 X-CROWD10 v2 §8.2.2 审定的 27 列宽表聚合 CSV（segment_name + dem_*/con_*/pri_sens_*/pri_dis_*/cha_*/lif_* + sample_count），每行 = 1 CrowdSegment（按 segment_name 分组），fieldProfiles 从列前缀推导，tagDictionary 草稿按列前缀映射 dimension（不走启发式），autoGenerated=true。返回 `{ dataset, tagDictionary, segments }`。② `POST .../auto-tag-dictionary`：适用于明细通道（D-CROWD2），复用 fieldProfiles，启发式维度映射（7 维度正则覆盖中英文），autoGenerated=true。③ `POST .../default-segment`：无条件全样本 segment（"全样本（自动）"、coverageRatio=1.0、autoGenerated=true）。**归一化层**（`crowd-normalize.ts`）：中→英枚举映射（gender→F/M, city_tier→T1/T1_NEW/..., discount→coupon/flash_sale/..., channel→xhs/douyin/offline/..., lifecycle→new/active/churned/..., scenario→weekend_outing/self_reward/...），未命中→原文+normalizationFailed:true，slash 多值逐段归一化。**红线**：端点 grep "openai|llm|chat" 空匹配；聚合上传不存原始行；错误返回体/日志不打印字段值样本。DB 迁移：crowd_datasets 加 `is_aggregate`、crowd_tag_dictionary/crowd_segments 加 `auto_generated`（ALTER TABLE 幂等）。**待 D 域前端接入**（D-CROWD11）。
