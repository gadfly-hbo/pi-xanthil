...
> **模块定位**：「日常」是 pi-Xanthil 的主分析入口，面向**一次性、轻量的随手探索与分析**，覆盖**从需求到行动**的完整链路：定义业务问题 → 准备数据 → 自由探索 → 与 AI 协作分析 → 输出报告 → 审核与汇报 → 沉淀策略与行动。需要围绕单一业务问题做经得起复核的深度专项时，请走**专题模块**（带门禁流水线 + 假设库飞轮）。
>
> 📅 更新日期：2026-06-20
>
...
进入路径：顶部主导航选中「**日常**」（`activeTab === "explore"`）。

二级 tab 按完整工作流顺序排列（`EXPLORE_SUB_TABS`）：

| # | 二级 tab | 一句话定位 | 与 LLM 关系 |
|---|---|---|---|
| 1 | **业务需求** | 把模糊的业务问题结构化为可分析的需求文档 | 调用 LLM 抽取结构化字段 |
| 2 | **原始数据** | 登记 / 浏览 `draw_data` 文件路径（红线域） | 文件名/结构可读，**原始行禁入 LLM** |
| 3 | **数据提取** | 用注册的提取工具把原始数据加工为聚合产物 | 工具只回灌聚合结果 |
| 4 | **聚合计算** | 浏览器内本地聚合（无 LLM 通道），可选生成 Python | **无 LLM** |
| 5 | **聚合数据** | 登记 / 浏览 `clean_data` 文件路径 | 允许进 LLM，UI 有琥珀色提示 |
| 6 | **数据探索** | 浏览器内拖拽式 BI（DuckDB-WASM） | **永久禁止** LLM 调用 |
| 7 | **数据分析** | 与 AI 对话进行分析，左侧附 `clean_data` 只读文档 | 主对话区，全 LLM |
| 8 | **报告输出** | 登记 / 浏览 `report` 输出目录 | 衍生产物，可读 |
| 9 | **汇报版本** | 把详细报告精炼为汇报版 + 故事线 | LLM 改写 |
| 10 | **报告审核** | 多模型对报告打分 + 给出修改建议 + 自动修复 | LLM 评审 |
| 11 | **黄金策** | 用商业框架（决策树/TOC/SWOT…）生成策略图 | 多模型并行 |
| 12 | **行动** | 把报告中的建议拆为可执行 action item | LLM + 任务管理 |
| 13 | **DLF** | DLF 模块管理（占位，即将推出） | — |
...
- 浏览并运行**已注册的提取工具**（`ExtractionTool`），把原始数据加工成聚合产物——这是「让 AI 看明细」的合规通道。
- 工具由 `/api/extraction-tools/:id/run` 执行，**只回灌聚合结果**，原始行级数据不进 LLM（UI 有 `ShieldCheck` 安全标识）。
- 可预览工具源码、运行记录（`ExtractionRun`）与产物。

### 操作步骤

1. 在工具列表选一个匹配任务的提取工具（如清洗 / 指标计算 / 匹配类）。
2. 指定输入（`draw_data` 路径）与参数，点运行。
3. 产物自动落到聚合层（`clean_data/`），随后可在「聚合数据 / 数据探索」继续使用。

### 案例

对 `orders_2025q4.csv` 运行「触达-订单匹配」提取工具，输出匹配明细的聚合表落 `clean_data/touched_orders.csv`——明细计算在工具内完成，LLM 只看到聚合结果。

---

## 4. 聚合计算（`aggregate_compute`）

### 功能

- **浏览器内本地聚合**通道（DuckDB/JS，**无 LLM**）：选数据集 → 配置分组维度、聚合操作（sum/avg/min/max/count）、时间粒度、过滤 → 直接算出聚合表。
- 内置数据集剖析（`profileDataset`），结果可复制 / 导出 csv。
- 可选「生成 Python」：把当前聚合配置转成等价 Python 脚本（`buildPythonPrompt`），便于落库或复现；脚本生成不读原始明细内容。

### 操作步骤

1. 读取本地数据集（`draw_data` 中的 csv/xlsx）。
2. 拖配维度 + 选聚合操作 + 设时间粒度 / 过滤。
3. 运行得聚合表，复制 / 导出，或登记到 `clean_data/`。

### 案例

把 `orders_2025q4.csv` 按 `channel` 分组、`gmv` 求 `sum`、时间粒度按周——本地算出渠道周度 GMV 表，导出后登记为聚合数据，供后续对话分析引用。
...
## 6. 数据探索（`data_exploration`） 🔒 永久无 LLM
...
> 以「Q4 复购下滑归因」为例，一次完整跑完日常模块核心 tab。

```
1. 业务需求           → 填项目背景 → 生成结构化需求 v1（拿到 dataNeeds.fields）
2. 原始数据           → 登记 orders_2025q4.csv / touch_list.csv（不进 LLM）
3. 数据提取/聚合计算  → 用提取工具或本地聚合把明细加工成聚合表（不泄露明细）
4. 聚合数据           → 写 复购口径.md / KPI 定义.md（允许进 LLM，左栏可见）
5. 数据探索           → 从业务需求点 channel 跳过来 → DuckDB 拖拽剖析 → 看到直播渠道异常
6. 数据分析（对话）   → 注入 Skill + 业务需求 → AI 调聚合工具 → 落 report/v1.md
7. 报告输出           → 浏览 v1.md，生成 HTML 发给业务方
8. 报告审核           → 多模型评审 36/50 → 自动修复 P0/P1 → 得到 v2.md（44/50）
9. 汇报版本           → 基于 v2 生成精简版 + 故事线 → 投屏汇报
10. 黄金策             → TOC + Porter 双图 → 主约束 = 老客触达通道单一
11. 行动              → 拆 8 条 action → 自动跑 3 条 SQL → 反馈回写项目记忆
```
