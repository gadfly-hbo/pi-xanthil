...
- [x] **规则记忆模块重构（clean-slate 重建）—— 全链交付完成（2026-06-19，11/11 卡 done）** — 统一 MemoryItem 模型 + 多信号检索(relevance/recency/feedback/typePriority) + 治理(时序衰减/supersede/防 poison/dedup) + 自动沉淀闭环(trace 蒸馏→门禁→自动入库 or 复核队列) + 统一管理面板(RulesPane 重建) + ChatPane 内联反馈 + KG 关系层/注入检查器/失败诊断。X-FINAL 收尾：typecheck+build+164 测试 + 闭环 E2E 烟测 6/6(沉淀→召回→注入→反馈压制→KG 投影→缓存前缀不回退)。**V-OBS 由总控收口(V-agent 已停用，见 §一)**。详见 `docs/记忆重构-任务派发.md`、`docs/wiki.html`。待归档 changelog。
