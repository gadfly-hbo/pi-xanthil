...
// 实验场三级子 tab（左侧竖栏）。标签不带「实验场」尾缀。skill/tool 已实装，其余为占位脚手架（后端后补）。
export const LAB_SUB_TABS: { id: SubTab; label: string }[] = [{ id: 'skill', label: 'skill' }, { id: 'tool', label: 'tool' }, { id: 'hooks_lab', label: 'hooks' }, { id: 'command_lab', label: 'command' }, { id: 'subagents_lab', label: 'subagents' }, { id: 'prompts_lab', label: 'prompts' }];
export const LAB_SUB_IDS = new Set<SubTab>(['skill', 'tool', 'hooks_lab', 'command_lab', 'subagents_lab', 'prompts_lab']);
