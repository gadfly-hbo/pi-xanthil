...
- 最近更新：2026-06-29 · **X-CROWD12 全链验收完成（总控自做，the-crowd 二阶段收口）**
- 进度：
  - **the-crowd 主线状态**：X-CROWD0/1/2/3/4 · E-CROWD5 · D-CROWD6/7 · E-CROWD8 · X-CROWD9 · X-CROWD10 v2 · D-CROWD10 v2 · E-CROWD11 v2 · D-CROWD11 v3 · **X-CROWD12** 全部完成并通过总控终审。the-crowd 已完成两阶段建设：一阶段（占位页 → 人群画像资产库）+ 二阶段（CSV 模板下载 + 双通道一键画像 + 中→英归一化层）。
  - **X-CROWD12 全链验收（总控自做）**：① 安全审计 4 项 grep 全空（聚合通道零 LLM 调用、数据探索子树隔离、prompt builder 无 raw row 入口、模板路由白名单防穿越）。② 全测全绿（server 110/110 含 crowd-aggregate 24 + crowd-segment 27 + crowd-import 4 + crowd-profile-runner + crowd-e2e-smoke + crowd-x-crowd12-acceptance 6 + simulation-lab 28；subagent-evaluation 全绿；web typecheck + build 全过）。③ 新增 `server/src/crowd-x-crowd12-acceptance.test.ts` 6 用例固化二阶段交付：模板 4 文件 BOM + 列结构、demo-detail 40 行 + 各字段 ≥2 枚举、归一化层入库英文枚举、聚合通道 demo-aggregate 6 行 → 6 segments、明细 4 步链路 + prompt 不含 PII（user_id/phone/email/openid/unionid/身份证/手机号）、聚合 prompt 仅含聚合摘要。④ Subagent 发布契约守住（toolIds:[] + dataScope:clean_data）。⑤ Simulation lab 联动未受影响（source-guard 测试通过）。
  - **D-CROWD11 v3 返修（R1.1）**：`defaultModel` 照搬 GoldenStrategyPane.tsx 项目惯例 4 级 fallback：`DEFAULT_MODEL("minimax-cn/MiniMax-M3")` → `isDefault` → `models[0]` → 常量兜底。listModels 失败/空数组时画像 step 不再 100% 红。
  - **D-CROWD11 v2 返修**：① R1 修复：`DatasetImporter` mount 时 `api.listModels()` 获取真实 model 列表。② R2 修复：聚合通道严格遵循 §8.3.2 v2 契约——仅 2 步自动（导入+分群），不自动跑画像 step；Stepper 只显示 2 步；聚合安全提示条改为"导入后请在分群列表逐个点「生成画像」"。明细通道保持 4 步全自动（§8.3.1）。
  - **D-CROWD11（双通道上传 + 一键画像）**：`DatasetImporter.tsx` 重写为双通道（明细上传/聚合上传 Tab 切换），每个通道有独立安全提示条。新增「上传后自动生成画像」toggle（默认开）。`Stepper` 组件实时展示进度，每步 running/done/failed 状态反馈。任一步失败保留已成功资产、回退手动模式。`CrowdPane.tsx` segment 卡片新增 `autoGenerated` 徽章（灰色 "auto" 标签 + hover 提示）。
  - **D-CROWD10 v2 返修（5 条）**：① 明细模板枚举改中文展示值（女/男/其他、新一线、流失等），server 归一化对照表就绪；多值分隔符 `/`。② 聚合模板改为 27 列固定宽表（dem_/con_/pri_sens_/pri_dis_/cha_/lif_ 前缀），demo 6 行覆盖多 segment。③ `routes/data.ts` 删除 `readFileSync(filePath)` 死代码。④ `DatasetImporter.handleTemplateAction` 整个 try 期间置 `importing=true`，finally 清除。⑤ 两份 README「模板字段说明」按 v2 重写（明细 10 字段中英对照 + 聚合 27 列前缀说明）。
  - **X-CROWD9（总控验收审计）**：新增 `server/src/crowd-e2e-smoke.test.ts`，隔离数据目录跑通 mock 标签数据 → dataset → 标签字典 → segment → profile/persona → subagent draft → SimulationLab crowd life form → ProfileFeedback 回写 → 采纳生成新 ProfileVersion。smoke 同时断言：`user_id` 小样本 TopN 不暴露、profile prompt 不含 `user_id/u_001`、simulation prompt 不含 `toolIds/draw_data/dataset`、crowd 来源 subagent template 仍为 `dataScope=clean_data`。
  - **安全补丁（X-CROWD9 发现）**：`computeFieldProfiles()` 对 `row_id/record_id/user_id/phone/email/openid/主键/手机号` 等标识类字段无条件隐藏 `topValues`，不再只依赖高基数阈值；`coerceSubAgentTemplate()` 保留 `origin/crowdProfileId/crowdProfileVersionId`，避免 crowd 来源模板经 `/api/subagents` 保存后丢失来源追溯。
  - **E-CROWD8（SimulationLab 联动）**：SimulationRunResult 增加稳定 `id=simulation_${timestamp}`；crowd feedback 回写带 `sourceRunId/profileVersionId/sourceLifeFormId`，后续可追溯到具体模拟实验与生命体。
  - **D-CROWD6（CrowdPane 五区界面）**：`CrowdPane.tsx` 重写为平铺布局（~350 行）—— Zone 1 顶部概览（数据集/分群/画像/已发布计数 + 零原始行安全徽章）、Zone 2 数据导入（DatasetImporter + 数据集列表 + 空态引导）、Zone 3 标签字典（TagDictionaryEditor）、Zone 4 人群分群（分群列表 + SegmentBuilder + 空态引导）、Zone 5 画像侧写（ProfileViewer + 已发布状态标记）。选中数据集时展示 Zones 3–5，未选中时仅展示导入区。替代原有 datasets→detail 两级导航为单页平铺。
  - **D-CROWD7（subagents 联动）**：`ProfileViewer.tsx` 新增「发布为子 agent 模板」动作——调 `createCrowdSubAgentDraft` 生成 draft → 确认对话框（展示名称/persona/安全约束）→ `listSubAgents` + `saveSubAgents` 追加到 subagents.json。新模板 `origin=crowd_profile`、`toolIds=[]`、`dataScope=clean_data`。发布后 profile 卡片显示「已发布」徽章 + templateId。`SubAgentManagementPane.tsx` 左列表按 origin 显示来源标签（人群画像/系统/手动），编辑器显示 origin badge + profileId 回跳码。
  - **D-CROWD1（数据底座）**：6 张表 + 25 路由 + 23 client 方法（不变）。
  - **D-CROWD2（导入字典）**：DatasetImporter + TagDictionaryEditor + computeFieldProfiles（不变）。
  - **D-CROWD3（分群规则引擎）**：crowd-segment.ts + SegmentBuilder + 24 单测（不变）。
  - **D-CROWD4（画像版本）**：ProfileViewer + adoptFeedbackToVersion + rollbackProfileToVersion（不变）。
  - **非 the-crowd 历史改动仍在工作区**：D-AGING2、D-EVOLVE2、D-QEVAL3、D-QEVAL1、MEM-MAINTAIN/PROMOTE-UI、V-DLF2/D-DLF3、D-MONITOR-TARGET3、D-SAFEDISTILL1 等仍等待用户 review 后手动提交；本次未清理或回滚。
- 校验：
  ...
  - 数据探索红线 grep（DataExplorationPane.tsx + data-exploration/ + insights.ts + joins.ts + profiling.ts + crowd/ + CrowdPane.tsx）：✅ 0 匹配
  - `node --experimental-strip-types --test server/src/crowd-e2e-smoke.test.ts server/src/simulation-lab.test.ts server/src/crowd-profile-runner.test.ts server/src/crowd-import.test.ts server/src/crowd-segment.test.ts`：✅ 80/80 全过
- 下一步（接续优先级）：
  - ① 用户 review 后手动提交；the-crowd 二阶段（X-CROWD10~12 + D-CROWD10/11 + E-CROWD11）建议作为独立提交组，避免和历史 Health/monitor/eval/memory 改动混在同一 commit。
  - ② 浏览器人工点检 the-crowd 关键 UI（自动 smoke 已覆盖数据链路，人工点检只看交互和视觉）：a. 进入 the-crowd 点「下载明细模板」「下载聚合模板」浏览器下载；b. 点「试用明细示例」看 4 步 stepper 全绿；c. 点「试用聚合示例」看 2 步 stepper 全绿 + 分群列表出现 6 个 segment 带「auto」徽章；d. 编辑标签字典、创建分群、生成画像、发布 subagent、进入 SimulationLab 选择 crowd life form、回写反馈、回到 ProfileViewer 采纳反馈。
  - ③ 若继续 the-crowd，下阶段优先项（已有 follow-up）：(a) §8.3.3 分群失败「跳到 Zone 4」跨组件跳转；(b) Stepper 连线视觉小瑕疵；(c) 用户编辑 segment/tagDictionary 后 `autoGenerated` 前端清零；(d) `CrowdTagValueSummary.normalizationFailed` 落库写入（types 已定义，computeFieldProfiles 未透出）；(e) profile 人工编辑 / profile 搜索筛选 / subagent 来源回跳真实路由 / SimulationLab feedback 批量回写。
  - ④ 处理开放问题①（subagent evaluation 持久化向后兼容），再继续 D-QEVAL 相关卡。
- 阻塞 / 待确认：
  - 无硬阻塞。
- 开放问题：
  ...
  - ~~⑫ V-DLF2/D-DLF3 server 端 toolIds 防御~~：已由 `parseSimulationRunRequest()` 丢弃未知字段并由 `simulation-lab.test.ts` 覆盖；E-CROWD8/X-CROWD9 再次验证 simulation prompt 不含 `toolIds`。
