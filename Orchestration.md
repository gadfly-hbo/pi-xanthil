...
## 〇、模块命名映射（改名记录 · 2026-06-18）

> **顶部 tab 展示名已改，内部 id / DB kind / 路由一律不变（零迁移）。** 看到代码里的英文 id，按下表对应中文模块名；勿因新旧名混用而误解。

| 内部 id（不可改） | 当前显示名 | 曾用名 | 说明 |
|---|---|---|---|
| `explore` | **日常** | 探索 | 单次/日常数据分析（ChatPane）|
| `zhuanti` | **专题** | （AnaX 提升而来）| 深度专题：对话探索 + 流水线双模 + 完整数据/报告链路 |
| `multi` | **重复** | 工作流 | 可复用流程化分析；**其产物仍称「工作流 / flow」**（产物名与模块名解耦，勿改）|

**改名边界（防误伤）**：
- 只改「作为顶部 tab / 模块入口的展示名」。
- **绝不动**同字不同义的功能概念：`数据探索`(data_exploration)、`对话探索`(zhuanti anax_chat)、`EDA 探索性分析`、`workflow.json` / 多 agent 工作流引擎 / flow 产物。
- 内部 `explore`/`multi`/`zhuanti` id、`kind="multi"`、`source_name`、路由全程不变。

执行：MainHeader TABS label 已改（总控）；UI 展示文案精准点改派 E（见 wiki「模块改名·文案」卡）；本表为命名权威真源。
...
- [ ] **规则记忆模块重构（clean-slate 重建）—— 拆解已派发（2026-06-18）** — 用户决策：旧模块以手工维护为中心+注入静态选择→从未用起来，**推倒重来**(架构重构+范式升级)。范式锚点(2026)：经验记忆 + 多信号检索 + 治理防漂移(SSGM) + 可观测闭环。锁定默认：relevance=词法+信号混合(零新依赖)、沉淀=高置信低危自动入库+其余复核、旧表 drop&rebuild、`metric_definitions`/`business_contexts` 不动(adapter 当 fact 读入)。跨域归属(§二治理横切)：契约/迁移/签名口径/index legacy/终审=**X**；数据层 CRUD+检索+管理面板=**D**；蒸馏 runner+评估工具+调用点对齐+ChatPane 反馈=**E**；KG 关系层+注入检查器+失败诊断=**V**。**11 张卡已写入 `docs/wiki.html` TASKS(todo)**，详见 `docs/记忆重构-任务派发.md`。**派发顺序：先 X-CONTRACT(总控自做)→契约绿后放行阶段1→沉淀闭环→面板+可观测→X-FINAL 终审**。待用户从 wiki 复制分发。
