...
// D-POOL1 池化：list 返回全局池(纯池化，跨工作区可见)；workspaceId=null 视为全局模板恒启用；
// 非 NULL 模板入 workspace_memory_enablements(kind='prompt')，由本 ws 决定是否启用。
...
// ─── the-crowd（数据库 → 人群画像资产库 · X-CROWD0 契约）───────────────
// 安全口径：CrowdProfile 生成只能读取聚合摘要/标签解释/分群规则；不得把原始行级标签明细送入 LLM。
export type CrowdDatasetSource = "upload_csv" | "upload_excel" | "sql_import" | "manual" | "aggregate_upload";
export type CrowdDatasetStatus = "importing" | "ready" | "error";
export type CrowdTagSensitivity = "public" | "internal" | "sensitive";
export type CrowdProfileDimension =
  | "demographic"
  | "consumption_power"
  | "interest_preference"
  | "channel_preference"
  ...
  | "scenario_preference"
  | "custom";
export type CrowdSegmentRuleOperator = "eq" | "neq" | "in" | "not_in" | "range" | "exists" | "missing";
export type CrowdSegmentRuleLogic = "and" | "or";
export type CrowdProfileStatus = "draft" | "active" | "archived";
export type CrowdProfileFeedbackStatus = "pending" | "adopted" | "rejected";

export interface CrowdTagValueSummary {
  value: string;
  count: number;
  ratio: number;
...
export interface CrowdFieldProfile {
  field: string;
  inferredType: "string" | "number" | "boolean" | "date" | "unknown";
  missingCount: number;
  uniqueCount: number;
  topValues: CrowdTagValueSummary[];
...
export interface CrowdTagDictionaryEntry {
  id: string;
  workspaceId: string;
  datasetId: string;
  field: string;
...
export interface CrowdSegmentRuleCondition {
  field: string;
  operator: CrowdSegmentRuleOperator;
...
export interface CrowdSegmentRuleGroup {
  logic: CrowdSegmentRuleLogic;
  conditions: CrowdSegmentRuleCondition[];
}
...
export interface CrowdProfileContent {
  traits: string[];
  motivations: string[];
...
export interface CrowdProfile {
  ...
  publishedSubAgentTemplateId?: string;
  createdAt: number;
  updatedAt: number;
}

export interface CrowdProfileVersion {
  id: string;
  workspaceId: string;
  profileId: string;
  version: number;
  content: CrowdProfileContent;
  source: "generated" | "manual_edit" | "simulation_feedback";
  sourceFeedbackId?: string;
  createdAt: number;
}

export interface CrowdProfileFeedback {
  id: string;
  workspaceId: string;
  profileId: string;
  ...
  suggestions: string[];
  status: CrowdProfileFeedbackStatus;
  createdAt: number;
  reviewedAt?: number;
}

export interface CrowdProfileGenerationInput {
  segmentId: string;
  model: string;
...
export interface CrowdProfileGenerationResult {
  ...
  version: CrowdProfileVersion;
}

export interface CrowdSubAgentDraft {
  name: string;
  persona: string;
  source: "crowd_profile";
  crowdProfileId: string;
  crowdProfileVersionId: string;
}
...
export type DigitalLifeFormSource = "subagent_template" | "manual_persona" | "crowd_profile";
...
export interface DigitalLifeForm {
  ...
  crowdProfileId?: string; // source=crowd_profile 时为 CrowdProfile.id；仅复用 persona，不读取标签明细
  crowdProfileVersionId?: string;
}
...
export interface SubAgentTemplate {
  ...
  origin?: "manual" | "crowd_profile" | "system";
  crowdProfileId?: string; // origin=crowd_profile 时用于回跳 the-crowd；不代表数据读取授权
  crowdProfileVersionId?: string;
}
