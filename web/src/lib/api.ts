...
const legacyApi = {
  ...
  generatePresentationVersion: (payload: PresentationGenerateInput) =>
    fetch("/api/report-versions/generate", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(payload),
    }).then(json<PresentationTaskResult>),
  ...
  createBusinessContext: (workspaceId: string, payload: { category: BusinessContextCategory; title: string; content: string; source?: string; owner?: string; validFrom?: number | null; validUntil?: number | null }) =>
  ...
  updateBusinessContext: (id: string, payload: { category: BusinessContextCategory; title: string; content: string; source?: string; owner?: string; validFrom?: number | null; validUntil?: number | null }) =>
  ...
  listBusinessContextConflicts: (workspaceId: string) =>
    fetch(`/api/workspaces/${workspaceId}/business-contexts/conflicts`).then(json<BusinessContextConflict[]>),
  previewBusinessContextImport: (workspaceId: string, payload: { content: string; format: BusinessContextImportFormat }) =>
    fetch(`/api/workspaces/${workspaceId}/business-contexts/import/preview`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) }).then(json<BusinessContextImportPreview>),
  commitBusinessContextImport: (workspaceId: string, payload: { rows: BusinessContextImportRow[]; enable?: boolean; conflictPolicy?: "skip" | "create_version" }) =>
    fetch(`/api/workspaces/${workspaceId}/business-contexts/import/commit`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) }).then(json<BusinessContextImportCommitResult>),
  exportBusinessContexts: (workspaceId: string, enabledOnly = true, format: BusinessContextImportFormat = "json") =>
    fetch(`/api/workspaces/${workspaceId}/business-contexts/export?enabledOnly=${enabledOnly ? "true" : "false"}&format=${format}`),
  ...
  // 知识库「收集」联网聊天（X-COLLECT3）：独立于业务工作区的全局收集会话 + 文件夹。
  listCollectSessions: () =>
    fetch(`/api/collect/sessions`).then(json<CollectSession[]>),
  createCollectSession: (title: string, folderId?: string | null) =>
    fetch(`/api/collect/sessions`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ title, folderId: folderId ?? null }),
    }).then(json<CollectSession>),
  renameCollectSession: (id: string, title: string) =>
    fetch(`/api/collect/sessions/${id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ title }),
    }).then(json<CollectSession>),
  setCollectSessionFolder: (id: string, folderId: string | null) =>
    fetch(`/api/collect/sessions/${id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ folderId }),
    }).then(json<CollectSession>),
  deleteCollectSession: (id: string) =>
    fetch(`/api/collect/sessions/${id}`, { method: "DELETE" }).then(json<{ ok: boolean }>),
  listCollectFolders: () =>
    fetch(`/api/collect/folders`).then(json<CollectFolder[]>),
