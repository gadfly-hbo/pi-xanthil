...
- 写测试或执行验证的 agent 对自己在本任务中创建的临时运行时数据、临时文件、一次性脚本、一次性截图或调试产物负直接清理责任；应在同一任务 handoff 前完成清理、复验并报告，不应默认交给 Controller 另开清理任务。
- 需要用户确认的清理项，worker 必须在 handoff 中列出精确对象、来源、删除方式和风险；一旦用户或任务 brief 已明确授权，worker 应在原任务的下一轮修正中完成清理，而不是要求 Controller 代删。
- 只有在清理动作超出原任务授权、目标对象无法证明归属、正式 contract 拒绝删除、需要额外权限/人工决策，或清理本身已经形成独立风险时，Controller 才创建单独 cleanup 任务。
- 清理完成后，必须重新运行相关正式测试、typecheck、lint 或最小 smoke 验证，并报告结果和未验证项。

### 使用方式

1. 在功能实现和正式验证完成后，检查本轮 diff、未跟踪文件、运行产物和验证写入的临时业务数据。
2. 按“删除、保留、待确认”分类列出临时代码及证据。
3. 获得用户确认或任务 brief 已授权后，由产生这些对象的 worker 在同一任务内执行清理，不扩大到未授权的历史遗留范围。
...
验证任务通过正式 API 创建的临时项目、临时记录或临时文件，如果任务 brief 已限定对象和删除 contract，验证 agent 必须在 handoff 前按 contract 删除并证明删除前后状态；Controller review 只复核证据，不应常规另派 cleanup 任务。
