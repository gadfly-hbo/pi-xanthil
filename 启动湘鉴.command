...
cleanup() {
  ...
  lsof -ti tcp:5173 2>/dev/null | xargs kill 2>/dev/null
  lsof -ti tcp:8787 2>/dev/null | xargs kill 2>/dev/null
}
trap cleanup INT TERM EXIT

# 等待前端就绪（最多 60s）后打开浏览器
echo -n "⏳ 等待前端就绪"
for i in {1..60}; do
  if curl -s -o /dev/null "$WEB_URL" 2>/dev/null; then
    echo " 就绪！"
